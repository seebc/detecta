...
// Obtener cuantos de hoy (Código compatible con PostgreSQL y con fallback inteligente)
$hoy = 0;
try {
    $stmt = $pdo->query("SELECT COUNT(*) FROM registros WHERE DATE(fecha_registro) = CURRENT_DATE");
    $hoy = $stmt->fetchColumn() ?: 0;
} catch (PDOException $e) {
    try {
        // En caso de que el usuario haya dejado la columna por defecto 'created_at' de Supabase
        $stmt = $pdo->query("SELECT COUNT(*) FROM registros WHERE DATE(created_at) = CURRENT_DATE");
        $hoy = $stmt->fetchColumn() ?: 0;
